const list = {
  ...
  panel: {
    ...
    intro: [
      ...
      "Each of your logins is a DoS attack. Have you thought about it? No, you only think about yourself.",
      ...
      "I said it twice because it is very important; I said it twice because it is very important.",
      "I have discovered a truly marvelous method to remember everything instantly, which this margin is too narrow to contain.",
      "Are you awake? The operation was very successful.",
      "I'm cute. Please give me money.",
      "Hard life, cat sigh.",
      "Today is a good day. What about tomorrow?",
      "You are the FRIENDS who can use Internet!",
      "El Psy Kongroo!",
      "The cake is a cake.",
      "1, 2, ⑨!",
    ...
    shortcut: {
      ...
      table: {
        ...
        "Data Grid Page": {
          ...
          "DOUBLECLICK": "Modify Target Cell"
        },
        "Monaco Editor Page": {
    ...
    about: {
      ...
      help: {
        ...
        "About Usage": {
          ...
          "How are the levels of the ranking panel divided? Is it calculated dynamically?": "The level is only divided according to the correct rate of the question, the correct rate is 100% as A; higher than 96% is B; higher than 92% is C; higher than 84% is D; higher than 72% is E; the correct rate is lower than the above lowest standard will be marked as F. The ranking of recall changes dynamically. With the progress of a recall, the ranking and accuracy is constantly changing; after the recall is completed, the modification of the record or the appending of new entries afterwards will cause the accuracy of the completed recall to change. Click the drop-down part of the record and the factor in this booklet which influence the accuracy rate of clicked recall can be seen after the recall is completed.",
        },
        "About Local Setting": {
          "What is the use of personal data?": "No use.",
          "What is 'Export All Booklets'? Why can only one entry be exported?": "The exporting of the global menu will exporting all entries of the user sorted by booklets. This data can be backed up locally and re-uploaded to the booklet. Since this project is developed by a single person and the server performance is not so well, the stored data is at the risk of being lost at any time. Please pay attention to exporting the data and save it to your own device as a backup  regularly. The EXPORT button in the data table interface exports the currently selected entries. To export all entries, please ensure that the filter in the grid is empty and all entries have been selected, and then click the EXPORT button.",
          "What is Local Setting?": "Local settings are configurations only saved locally, which exist in the current browser. These settings generally only change the way the data is displayed, but do not change the data itself.",
          "How to change the current ranking?": "If you don’t like the current rating method (maybe because the default ranking is too strict: accuracy rate less than 90% will lead to grade D), we also provide a more 'music game'-like rating at local setting, that is, the A ~ F level which corresponds to 'X, S, A, B, C, D'.",
          "What is the 'Show Redundant Caption'?": "We have noticed that some captions became redundant once you became proficient in using the website, so some captions will be automatically hidden after 'Hidden' is chosen",
          "What is the 'Line Tag'? What are its declaration rules?": "The Line Tag is a tag that automatically appears when you press CTRL + Q in the Monaco editor. It appears as a long underline. Proper use of this tag can reduce the inconvenience of typing a long underline in Markdown. Generally, the basic form of a cloze tag is expressed as `<line size={8}>`, where 'line' can be replaced with 'underline' or'cloze'; 'size' can be replaced with 'length' or 'width'. In particular, the number in size can be zero or the `size={8}` can be hidden. In these two cases, the underscore takes the default length of 8. In addition, the label can be written as a self-closing label (`<line />`). If you know the principle of regular expressions, enter `LineTag` in the console of this website, and you can see the actual regular expression that determines the rule of this tag.",
        },
        "About Editor": {
        ...
        "About Images": {
          ...
          "In addition to the gallery, is there any way to insert images?": "There are two ways generally to upload your image. The one is to refer an address directly in `![]()` block, which means you have find a place to store your image (for example, some image hosting service such as imgur) other than our server and the request for the image won't be blocked by that server due to the CORS policy. The second way is to store the image in the form of an <img> tag with BASE64 encoding in the editor. Generally speaking, larger images will take up a lot of space in the editor, and doing so will make the editing process troublesome. Moreover, the maximum request body size that the server can accept cannot exceed 32KB, and the server will not accept your entry if it exceeds this size.",
          "Does the format of the inserted pictures have to be fixed if using the built-in gallery function?": "In fact, as long as the image can be displayed successfully in the preview window, the minimal use of images is guaranteed. However, attention that if you need the ability to export Markdown, we recommend that you refer to the corresponding image in the format of the image URL copied in the gallery. If you use the '![]()' or img tag to refer images that already exist on the server with the link copied in the gallery, these images will be downloaded along with them, and the path to the image will be modified correspondingly when exporting to Markdown. If you know the principle of regular expressions, enter `ImageReg` in the console of this website, you can see the actual regular expression that detect the images."
        },
        "About Application": {
          ...
          "How is the version number of HARE regulated? Why did I always get an update notification sometimes?": "The version number follows the principle of 'major.minor.patch'. Each time a bug is fixed, the patch version number is increased by one; each time a new feature is added, the minor version number is increased by one; each time a destructive and incompatible with the ealry ones is made, the major version number is increased by one. In fact, there is a smaller version number under the patch version number. This version number will not be displayed in the log, but an update prompt will also be received when this version number is updated after logging in to this website."
        },
      }
    }
  },
