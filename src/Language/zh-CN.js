  ...
  popup: {
    ...
    about: {
      ...
      info: {
        ...
        "React KaTeX": ["React LaTeX 公式渲染", "2.0.2"],
      ...
      help: {
        ...
        "关于使用方法": {
          ...
          "结算界面的等级是怎么划分的？它是动态计算的吗？": "等级只按照问题正确率来划分，正确率 100% 为 A；高于 96% 为 B；高于 92% 为 C；高于 84% 为 D；高于 72% 为 E；正确率低于上述最低标准的记为 F。如果您不喜欢现在的评级方式，我们也提供一种更加「音游」式的评级，即将 A ~ F 等级对应到 「X、S、A、B、C、D」。结算界面的等级是动态变化的，随着一次回忆的进行，等级在不断变化；完成回忆后，事后对于记录的修改或者添加新的条目都会导致已经完成的记忆准确率发生变化。点击一次记录的下拉部分，可以看到在记录完成后由于记忆册变化而导致准确率变化的日志。"
        },
        "关于编辑器": {
