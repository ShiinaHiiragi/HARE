  ...
  popup: {
    ...
    about: {
      ...
      help: {
        ...
        "关于编辑器": {
          "如何在 MARKDOWN 编辑器内插入图片？": "一般来说我们有两种方法达成这个目标。第一种是直接使用 `![]()` 的方式引入图片，使用这种方式说明图片已经被您用某种方式存在了您指定的网址，而不需要存在我们的服务器上。同时您需要保证我们的 MARKDOWN 渲染器对于图片的请求不会被对方服务器由于 CORS 策略阻拦。第二种方式是让图片以 img 标签的形式存储 BASE64 编码在编辑器中。一般来说，较大的图片会占据编辑器的大量空间，这么做会让编辑过程变得麻烦。而且，服务器能接受的最大请求体大小不能超过 4MB，超过这个大小服务器并不会接受您的词条。",
        ...
        "关于此应用": {
          ...
          "HARE 的服务是否收费？服务器存储是否可靠？服务器提供多少存储空间？": "HARE 没有收费项目，但是相对的服务器并不是高性能服务器，因此服务提供不稳定。存储的数据可能有遗失的风险，请注意定期导出数据并保存到自己的设备上。默认对于每一位新用户，最多可以创建 16 个记忆组，每个记忆组最多可以创建 16 个记忆册，每个记忆册最多可以创建 64 个词条。每个词条装在请求体中不超过 4MB 即可存储到服务器中。如果达到这个上限，新建按钮将变灰，指示其不可用。",
