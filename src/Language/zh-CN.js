    ...
    about: {
      ...
      help: {
        "关于账号安全": {
          ...
          "为什么头像的图片网址有时会显示「INVALID ARGUMENT」？": "您可能右键在新标签页打开了您的头像。但是服务器不会泄露您的头像，请求头像需要验证您浏览器 Cookie 中的 UID 和令牌。如果您将图片网址传给了其他人，其他人由于没有您的令牌，不会得到图片。"
        },
        "关于使用方法": {
        ...
        "关于编辑器": {
          "如何在 MARKDOWN 编辑器内插入图片？": "一般来说我们有两种方法达成这个目标。第一种是直接使用 `![]()` 的方式引入图片，使用这种方式说明图片已经被您用某种方式存在了您指定的网址，而不需要存在我们的服务器上。同时您需要保证我们的 MARKDOWN 渲染器对于图片的请求不会被对方服务器由于 CORS 策略阻拦。第二种方式是让图片以 img 标签的形式存储 BASE64 编码在编辑器中。一般来说，较大的图片会占据编辑器的大量空间，这么做会让编辑过程变得麻烦。而且，服务器能接受的最大请求体大小不能超过 32KB，超过这个大小服务器并不会接受您的词条。",
        ...
        "关于此应用": {
          ...
          "HARE 的服务是否收费？服务器存储是否可靠？服务器提供多少存储空间？": "HARE 没有收费项目，但是相对的服务器并不是高性能服务器，因此服务提供不稳定。存储的数据可能有遗失的风险，请注意定期导出数据并保存到自己的设备上。默认对于每一位新用户，最多可以创建 16 个记忆组，每个记忆组最多可以创建 16 个记忆册，每个记忆册最多可以创建 64 个词条。每个词条装在请求体中不超过 32KB 即可存储到服务器中。如果达到这个上限，新建按钮将变灰，指示其不可用。",
          "HARE 是否开源？遇到错误提示或漏洞怎么办？": "HARE 目前暂不开源，如果遇到错误提示，请按照提示内容执行正确的操作；如果只有错误提醒而没有解决方案，这可能是漏洞，请将漏洞内容和复现方式告知作者。",
