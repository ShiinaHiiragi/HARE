    ...
    about: {
      ...
      help: {
        "关于账号安全": {
          ...
          "什么是会话？为什么会过期？": "会话是打开一个新的 HARE 标签页时生成的特殊字符串，如果同时在多处登录了同一个账号，那么会话保证只有最新打开的标签页是有效的页面，可以与服务器交互，而其他的页面一旦与服务器交互就会被返回「409 Conflict」的错误提示。会话保证了您在与服务器交互时，存储在浏览器的数据都是最新的。",
        ...
        "关于编辑器": {
          "如何在 MARKDOWN 编辑器内插入图片？": "一般来说我们有两种方法达成这个目标。第一种是直接使用 `![]()` 的方式引入图片，使用这种方式说明图片已经被您用某种方式存在了您指定的网址，而不需要存在我们的服务器上。同时您需要保证我们的 MARKDOWN 渲染器对于图片的请求不会被对方服务器由于 CORS 策略阻拦。第二种方式是让图片以 img 标签的形式存储 BASE64 编码在编辑器中。一般来说，较大的图片会占据编辑器的大量空间，这么做会让编辑过程变得麻烦。而且，服务器能接受的最大请求体大小不能超过 2M，超过这个大小服务器并不会接受您的词条。",
